...
const DonorRegister = () => {
  ...
  const [organization, setOrganization] = useState(""); // For registration
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const { data, error: upsertError } = await supabase
        .from("donors")
        .upsert({ email, name, phone, location, password, organization });
  ...
  return (
    <div className="container py-5 mt-5 form-container">
      <form className="mx-auto w-50">
        <h1 className="h3 mb-3 fw-normal text-center">Donor Register</h1>
        ...
        <div className="form-floating mb-3">
          ...
          <label htmlFor="floatingOrganization">Organization</label>
        </div>
        <div className="form-floating mb-3">
          <input
            type="password"
        ...
        <p className="mt-5 mb-3 text-body-secondary text-center">
          © 2024 | <a href="/login/donor">Already a member? Login now</a>
        </p>
      </form>
    </div>
  );
